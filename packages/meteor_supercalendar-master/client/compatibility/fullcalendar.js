...
function setMinHeight(element, height) {
	...
	element.each(function(i, _element) {
		_element.style.cssText += ';min-height:110px' + ';_height:' + height;
...
function AgendaView(element, calendar, viewName) {
	...
	function buildSkeleton() {
		...
		for (i=0; d < maxd; i++) {
			...
			s +=
				"<tr class='fc-slot" + parseInt(i/2) + ' ' + (!minutes ? '' : 'fc-minor') + "'>" +
...
function DayEventRenderer() {
	...
	function buildHTMLForSegment(segment) {
		...
		if (event.event_name){
			html +=
				"<span class='fc-event-title'>" + htmlEscape(event.event_name) + "1111</span>" +
				"</div>";
		} else {
			html +=
			"<span class='fc-event-title'>" + htmlEscape(event.title) + "1111</span>" +
			"</div>";
		}
